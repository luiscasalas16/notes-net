        ...
        public OrderShipmentStateMachine(ILogger<OrderShipmentStateMachine> logger)
        {
                ...
                When(OrderSubmitted)
                    .Then(context =>
                        logger.LogInformation(
                            "Monitoring Order Shipment: {OrderId}",
                            context.Saga.CorrelationId
                        )
                    )
                    .Schedule(
                        MonitorTimeout,
                        context => new MonitorOrderShipmentTimeout
                        {
            ...
            During(
                ...
                When(OrderShipped)
                    .Then(context =>
                        logger.LogInformation(
                            "Shipment Completed (overdue): {OrderId}",
                            context.Saga.CorrelationId
                        )
                    )
                    .TransitionTo(ShipmentComplete)
            ...
            During(
                ...
                When(OrderSubmitted)
                    .Then(context =>
                        logger.LogInformation(
                            "Order Shipment (already shipped): {OrderId}",
                            context.Saga.CorrelationId
                        )
                    )
            );
