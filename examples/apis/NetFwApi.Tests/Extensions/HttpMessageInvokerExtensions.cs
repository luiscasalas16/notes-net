...
namespace NetFwApi.Tests.Extensions
{
    public static class HttpMessageInvokerExtensions
    {
        ...
        public static async Task<HttpResponseMessage> PutAsJsonAsync<TValue>(this HttpMessageInvoker client, string requestUri, TValue value)
        {
            var request = new HttpRequestMessage(HttpMethod.Put, requestUri) { Content = new StringContent(JsonConvert.SerializeObject(value), Encoding.UTF8, "application/json") };

            return await client.SendAsync(request, CancellationToken.None);
        }

        public static async Task<HttpResponseMessage> DeleteAsync(this HttpMessageInvoker client, string requestUri)
        {
